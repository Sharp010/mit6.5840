...
func TestStaticShards(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		va[i] = randstring(20)
		ck.Put(ka[i], va[i])
		Debug_(dClient, "CHECKPOINT 2-%d", i)
	}
	Debug_(dClient, "CHECKPOINT 3")
	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
	}
	Debug_(dClient, "CHECKPOINT 4")
	// make sure that the data really is sharded by
	// shutting down one shard and checking that some
	// Get()s don't succeed.
	cfg.ShutdownGroup(1)
	cfg.checklogs() // forbid snapshots
	Debug_(dClient, "CHECKPOINT 5")
	ch := make(chan string)
	for xi := 0; xi < n; xi++ {
		...
		go func(i int) {
			v := ck1.Get(ka[i])
			Debug_(dClient, "CHECKPOINT 5-Sid %d", key2shard(string(rune(xi))))
			if v != va[i] {
...
func TestSnapshot(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		va[i] = randstring(20)
		ck.Put(ka[i], va[i])
		Debug_(dClient, "CHECKPOINT 1-%d", ka[i])
	}
	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		Debug_(dClient, "CHECKPOINT 2-%d", ka[i])
	}
	...
	for i := 0; i < n; i++ {
		...
		x := randstring(20)
		ck.Append(ka[i], x)
		Debug_(dClient, "CHECKPOINT 4-%d", key2shard(ka[i]))
		va[i] += x
	}
	...
	for i := 0; i < n; i++ {
		...
		x := randstring(20)
		ck.Append(ka[i], x)
		Debug_(dClient, "CHECKPOINT 6-%d", key2shard(ka[i]))
		va[i] += x
	}
	Debug_(dClient, "CHECKPOINT 7")
	time.Sleep(1 * time.Second)
	...
	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		Debug_(dClient, "CHECKPOINT 8-%d", key2shard(ka[i]))
	}
	...
	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		Debug_(dClient, "CHECKPOINT 9-%d", key2shard(ka[i]))
	}
...
func TestConcurrent1(t *testing.T) {
	...
	ff := func(i int) {
		...
		for atomic.LoadInt32(&done) == 0 {
			x := randstring(5)
			ck1.Append(ka[i], x)
			Debug_(dClient, "CHECKPOINT 1-%s", ka[i])
			va[i] += x
	...
	Debug_(dClient, "CHECKPOINT 10")
	for i := 0; i < n; i++ {
		<-ch
		Debug_(dClient, "CHECKPOINT 9-%d", i)
	}
	Debug_(dClient, "CHECKPOINT 8")
	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		Debug_(dClient, "CHECKPOINT 7-%s", ka[i])
	}
